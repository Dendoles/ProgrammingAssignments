...
#pragma region OpenGLDrawFunctionality
	...
	{
		ClearBackground(40.0f / 255.0f, 42.0f / 255.0f, 54.0f / 255.0f);
	}
...
#pragma region CollisionFunctionality
	...
	bool IsPointInCircle(const Point2f& pointPos, const Circlef& circle)
	{
		if (GetDistance(pointPos, circle.center) > circle.radius)
		{
			return false;
		}
		return true;
	...
	bool IsPointInRect(const Point2f& pointPos, const Rectf& rectangle)
	{
		if ((rectangle.left <= pointPos.x &&
			rectangle.left + rectangle.width >= pointPos.x) &&
			(rectangle.bottom <= pointPos.y &&
				rectangle.bottom + rectangle.height >= pointPos.y))
		{
			return true;
		}
		return false;
	...
	{
		const Point2f l1{ rectangle1.left , rectangle1.bottom + rectangle1.height };
		const Point2f r1{ rectangle1.left + rectangle1.width, rectangle1.bottom };
		const Point2f l2{ rectangle2.left , rectangle2.bottom + rectangle2.height };
		const Point2f r2{ rectangle2.left + rectangle2.width, rectangle2.bottom };
	...
	bool isOverlapping(const Circlef& circle1, const Circlef& circle2)
	{
		...
		if (GetDistance(circle1.center, circle2.center) < radius) return true;
		return false;
	}
...
//ArrayFunctionality
	...
	void ArrayShuffle(int* pArray, const int arraySize, int numberOfSwaps)
	{
		...
		for (int i{}; i < numberOfSwaps; i++)
		{
			...
			ArraySwap(index1, index2, pArray);
		}
	}
